...
| RF05 | Converter observações em parâmetros por regras determinísticas | RNF08 — não é wrapper de IA | `regras.ts` + `ciclos.ts` + `ancora.ts` (D-18) + EF `fechar-ciclo` (D-09/D-10); protótipo `mockApi.fecharCiclo` | `regras.test.ts` 11 · `ciclos.test.ts` 10 · `mockApi.test.ts` "PENDENTE"/"RN06" · cobertura 97 % | `efca920`, `c35a1d9` (motor); P2.1–P2.5 e tela: *(commit desta entrega)*; EF pendente |
...
| RF09 | Camada determinística de adaptação | RNF08 | D-10/D-18 · `adaptador.ts` no motor + `MaterialAdaptado.tsx` (CSS a partir de `parametros`) | `adaptador.test.ts` 9/9: determinístico; blocos ≤ limite; nada se perde; etapa única sem conectores | *(commit desta entrega)* |
...
| RF11 | Revisão, edição e aprovação humana | RN04 — "e se a IA distorcer?" | D-12 · rascunho privado; `docente_aprova_ou_descarta_material`; tela `RevisarMaterial.tsx` | `mockApi.test.ts` "D-12": responsável lê RASCUNHO → não encontrado; após APROVADO → lê; `paginas.test.tsx` fluxo gerar→aprovar | *(commit desta entrega — mock)*; RLS pendente |
| RF12 | Versão web acessível + impressão | RNF03, RNF05 | D-10 · `MaterialAdaptado.tsx` + `print.css` | `componentes.test.tsx` "MaterialAdaptado": `data-contraste=7`, lista de etapas, 0 violações axe; `scripts/contraste.mjs` | *(commit desta entrega)* |
...
| RF14 | RBAC + permissões granulares | RN02, RNF04, RNF09 | RLS `0002` + revisões D-11/D-12; `mockApi.ts` (D-17) | **Docente lê `notas_clinicas` com token válido → 0 linhas / insert → 403** (Exemplo 3) — `mockApi.test.ts` "RN02 ⭐" + `paginas.test.tsx` "Acesso negado (RN02)" | `c35a1d9` (PoC PGlite); *(commit desta entrega — 34 cenários no mock)*; PostgREST pendente (P1.12) |
...
| RNF03 | WCAG 2.2 AA + NBR 17225 | Público-alvo | `docs/wcag-2.2.md`, `docs/acessibilidade.md`; tokens verificados; axe-core em componentes e telas; NVDA manual | 0 violações A/AA em 59 testes (jsdom); `npm run contraste`; NVDA (P6.7) pendente | *(commit desta entrega)* |
...
| D-15 design system em CSS com tokens (sem Tailwind) | `apps/web/src/styles/tokens.css`, `docs/ux-ui.md` §3 | *(commit desta entrega)* |
| D-16 autenticação em modo demonstração | `apps/web/src/pages/Entrar.tsx`, `hooks/useSessao.tsx` | *(commit desta entrega)* |
| D-17 contrato `PeiVivoApi` + mock com permissões | `apps/web/src/services/` | *(commit desta entrega)* |
| D-18 `adaptador.ts` antes das Edge Functions | `packages/motor-adaptacao/src/adaptador.ts` | *(commit desta entrega)* |
